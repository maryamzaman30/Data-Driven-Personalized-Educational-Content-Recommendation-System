...
def load_hybrid_model(path='models/svd_hybrid_model_best.pkl'):
    ...
    if recommender.user_factors is None or recommender.item_factors is None:
        recommender.fit(model_package['train_matrix'], model_package.get('content_similarity'))
...
def load_all_models():
    from src.utils.preprocessing import load_clean_data, load_content_model
