...
def get_subject_category_mapping():
    subject_mapping = {
        ...
        'vocabulary_business': list(range(71, 102)),
        'vocabulary_general': list(range(108, 146)),
        'reading_comprehension': list(range(148, 178)),
        'listening_comprehension': list(range(187, 217)),
        'advanced_topics': list(range(220, 299))
    }
    tag_to_category = {}
...
def create_lecture_title(lecture_id, part, tags, video_minutes=None):
    ...
    duration_str = f" ({video_minutes:.1f} min)" if pd.notna(video_minutes) and video_minutes > 0 else ""
    return f"{part_name}: {subject}{duration_str}"

def enrich_recommendations_with_metadata(recommendations, lectures_df):
    enriched_recs = []
    for rec in recommendations:
        # If recommendation is already enriched, just add it
        if isinstance(rec, dict) and all(key in rec for key in ['item_id', 'score', 'title', 'part', 'part_id', 'subjects', 'duration_minutes', 'type']):
            ...
            # Handle raw recommendations (item_id, score)
            if isinstance(rec, (list, tuple)) and len(rec) == 2:
                item_id, score = rec
                lecture_row = lectures_df[lectures_df['lecture_id'] == f"l{item_id}"]
                ...
                # If we can't handle the recommendation format, skip it
                continue
